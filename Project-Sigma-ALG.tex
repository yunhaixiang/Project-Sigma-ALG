...
as the \textit{zero-locus} of $S$. Subsets of $\mathbf A^n(k)$ that are the \textit{zero-loci} of some $S$ are called (affine) \textit{algebraic}.
\end{definition}
...
\end{theorembox}
whose proof we will provide later. It is really this observation that is central to algebraic geometry. Next, we observe that $\mathcal{V}()$
\end{document}
