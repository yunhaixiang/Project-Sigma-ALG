...
\begin{definitionbox}
Let $\mathcal{C},\mathcal{D}$ be categories, then a $\mathcal{D}$-valued presheaf on $\mathcal{C}$ is a functor $\mathcal{C}^{\mathrm{op}}\rightarrow\mathcal{D}$.
\end{definitionbox}
Many examples of presheaves come from different classes of functions, for example, we can define $\mathscr{F}(U)=\mathscr{C}^0(U)$ as the class of continuous real-valued functions.
\end{document}
